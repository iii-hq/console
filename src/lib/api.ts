// Configurable engine connection settings via environment variables
const ENGINE_HOST = typeof window !== 'undefined' 
  ? (process.env.III_ENGINE_HOST || 'localhost')
  : (process.env.III_ENGINE_HOST || 'localhost');

const ENGINE_PORT = typeof window !== 'undefined'
  ? (process.env.III_ENGINE_PORT || '3111')
  : (process.env.III_ENGINE_PORT || '3111');

const WS_PORT = typeof window !== 'undefined'
  ? (process.env.III_WS_PORT || '31112')
  : (process.env.III_WS_PORT || '31112');

const DEVTOOLS_API = `http://${ENGINE_HOST}:${ENGINE_PORT}/_console`;
const MANAGEMENT_API = `http://${ENGINE_HOST}:${ENGINE_PORT}/_console`;
const STREAMS_WS = `ws://${ENGINE_HOST}:${WS_PORT}`;
